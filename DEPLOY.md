...
The landing page and the findings page are static, and GitHub Pages serves
them from `main` at https://himanshu748.github.io/voxrede/. `index.html` is
generated by `build_page.py` alongside `landing.html`, so regenerate and
commit both after a new run:

```bash
python build_page.py && python combined_build.py && python deck_build.py
```
